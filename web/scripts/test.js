        ...
        listView: {
            ...
            detailView: {
                ...
                tabFilter: function(args) {
                	var hiddenTabs = [];
                	g_worktype = args.context.test[0].workOrderType;
                	//alert(g_worktype);
                  if(args.context.test[0].status=="1"){//待审批
                ...
                tabs: {
                    ...
                    details: {
                        ...
                        preFilter: function(args) {
                            var hiddenFields = [];
                            if(args.context.test[0].workOrderType==newVolume){
                            	var iscustomized =getItemAttributeValue(args.context.test[0],"iscustomized");
                            	if(iscustomized==0){//非自定义磁盘大小的方案
                            		hiddenFields.push("diskSize");
                            	}
                            }else if(args.context.test[0].workOrderType==deployVirtualMachine){
                            	var dvmdiskSize =getItemAttributeValue(args.context.test[0],"diskSize");
                       ...
                       /* actions:{
                        ...
                        dataProvider: function(args) {
                        	...
                        	var renderObj = {};
                           if(args.context.test[0].status!=1){
                        	   var data = {
               						id : args.context.test[0].id,
               						cmsz : "yes",
               						step : 2
               					};
                        	   $.ajax({
                        	   ...
                        	   renderObj = args.context.test[0];
                           }
                          
        					// 初始化字段
        					var fields = viewFields(args.context.test[0]);
        					var obj = {};
        					$.extend(obj, renderObj);
        					
        					for(var i = 0;i<args.context.test[0].workItems.length;++i){
        						var workItem = args.context.test[0].workItems[i];

        						//控制 以Default View视图添加实例时 ProjectId字段不显示
        						if(workItem.attributeName=="projectid" && workItem.attributeValue=="" && args.context.test[0].workOrderType==deployVirtualMachine){
        						...
        						*/
        						if(fields[workItem.attributeName] && fields[workItem.attributeName].externalValue==1 && (fields[workItem.attributeName].valType=="text" || (fields[workItem.attributeName].valType=="enum" && !fields[workItem.attributeName].editable))){
        							$.ajax({
        								...
        								data : {
        									cmsz : "yes",
        									workordertype : args.context.test[0].workOrderType,
                    ...
                    approve: {
                        ...
                        preFilter: function(args) {
                                 ...
                                 if(args.context.test[0].workOrderType==newVolume){
                                	 g_iscustomized = getItemAttributeValue(args.context.test[0],"iscustomized");
                        ...
                        actions: {
                            add:{
                            	...
                            	action:function(args){
                                	 ...
                                	 var id = args.context.test[0].id;
                                	 var workOrder = args.context.test[0];
...
//                                         		$(window).trigger('cloudStack.fullRefresh');
                        ...
                        dataProvider: function(args) {
        					...
        					var renderObj = args.context.test[0];

        					// 初始化字段
        					var fields = initFields(args.context.test[0]);
        						...
        						$("select[name=zoneid]",$("#details-tab-approve")).bind("change",function(){
        							...
        							$.ajax({
        								...
        								data : {
        									...
        									domainid:args.context.test[0].domainId,
        									account:args.context.test[0].account,
        									zoneid : this.value
        								},
        								success : function(json) {
        									console.info(json);
