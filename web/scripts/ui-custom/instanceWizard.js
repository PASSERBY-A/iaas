                ...
                var dataGenerators = {
                    setup: function($step, formData) {
                        ...
                        return {
                            response: {
                                success: function(args) {
                                	$('select.pool:not(:first)').remove();
                                    // Zones
                                	
                                	/**
                                	 * added by steven 
                                	 * 在新增实例的时候第一步动态生成资源池选择下拉框的数据
                                	 */
                                     ...
                                     $.ajax({
                         				...
                         				success : function(json) {
	                         					 ...
	                         					 $.ajax({
	                                  				...
	                                  				success : function(data) {
	                                  					if(data&&data.resourcepoolpermission.resourcepoolpermissionobj.productionPool==1){
	                                  						$step.find('.select-zone select.pool').append(productPool);
	                                  					}
	                                  					if(data&&data.resourcepoolpermission.resourcepoolpermissionobj.testingPool==1){
	                                  						$step.find('.select-zone select.pool').append(testPool);
	                                  					}
	                                  				}
	                                  			});
                         					}else{
                         					...
                         					if($step.find('.select-zone select.pool').find("option").size()==0){
                         						$.ajax({
 	                                  				url : createURL("listProjectResourcePoolPermission"),
 	                                  				async : false,
 	                                  				data : {
                         					...
                         					if($step.find('.select-zone select.pool').find("option").length>0){
                        						 $.ajax({
                        				             url: createURL("listsubresource&cmsz=yes&response=json&resourcePoolId="+$step.find('.select-zone select.pool').find("option:eq(0)").val()),
                        				             dataType: "json",
                        				             async: false,
                        				             success: function(json) {
                        				                 zoneObjs = json.listzonesresponse.zone;
                        				                 if(zoneObjs&&zoneObjs.length>0){
                        				                 for(var i=0;i<zoneObjs.length;i++){
                        				                	 var option= $('<option>').attr({value: zoneObjs[i].id}).html(zoneObjs[i].name);
                        				                	 $step.find('.select-zone select.zone').append(option);
                        					                 }
                        				             }
                        				             }
                        				         });
                                     ...
                                     $step.find("select.pool").live('change',function(){
                                    		 $('select.zone:last').empty();
                                 		 $.ajax({
                                              url: createURL("listsubresource&cmsz=yes&response=json&resourcePoolId="+$(this).val()+"&d="+(new Date()).getTime()),
                                              dataType: "json",
                                              async: true,
                                              success: function(json) {
                                                 var zones = json.listzonesresponse.zone;
                                                  ...
                                                  for(var i=0;i<zones.length;i++){
                                                 	 var option=$('<option>').attr({value: zones[i].id}).html(zones[i].name);
                                	...
                                	/**
                                	 * updated by steven
                                	 * 将原有select修改为带class属性的select.zone  以区分select-zone下面两个不同的select选择器
                                	 * select>select.zone
                                	 */
                                     /*
                                    $(args.data.zones).each(function() {
                                        $step.find('.select-zone select.zone').append(
                                            $('<option>')
                                            .attr({
                                                value: this.id,
                                                'wizard-field': 'zone'
