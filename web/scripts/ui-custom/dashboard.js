    ...
    cloudStack.uiCustom.dashboard = function() {
        ...
        $dashboard.find('.view-all').click(function() {
            ...
            if ($(this).hasClass('network')) $('#navigation li.network').click();
                ...
                $browser.cloudBrowser('addPanel', {
                    title: $dashboard.hasClass('admin') ?($(This).parent().parent().hasClass('message')?'消息':'Alerts') : 'Events',
                    maximizeIfSelected: true,
                    complete: function($newPanel) {
                        $newPanel.listView({
                            $browser: $browser,
                            context: cloudStack.context,
                            listView: $dashboard.hasClass('admin') ?($(This).parent().parent().hasClass('message')?cloudStack.sections.events.sections.messages.listView:cloudStack.sections.events.sections.alerts.listView) : cloudStack.sections.events.sections.events.listView // Users cannot see events
                        });
                    }
                });
            };
