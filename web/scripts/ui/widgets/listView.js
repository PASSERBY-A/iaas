...
(function($, cloudStack, _l, _s) {
    ...
    var createHeader = function(preFilter, fields, $table, actions, options) {
        ...
        var appendFields = options.appendFields;
       //console.info(options);
	...
	//added by Li Manxin
      if(appendFields && !$.isFunction(appendFields) && !appendFields.noCompact && !uiCustom){
    	
    	  $.each(appendFields,function(i,v){
    		  $thead.find('tr').append(
                      $('<th></th>')
                      .html(_l(v.label))
                  );
    ...
    var createSearchBar = function($toolbar, listViewData) {
       /* var $search = $('<div></div>').addClass('text-search reduced-hide');
        ...
        if (listViewData.advSearchFields != null && !(typeof(listViewData.advSearchFields)=='boolean' && listViewData.advSearchFields==false)) {
        ...
        $(data).each(function() {
            ...
            $.each(fields, function(key) {
                ...
                if (field.editable) {
                    ...
                    // add by huang xianjin 
                    // listview checkbox field
...
//                            .addClass('button')
            ...
            if ((listViewArgs.activeSection && listViewArgs.sections && listViewArgs.sections[listViewArgs.activeSection].listView && listViewArgs.sections[listViewArgs.activeSection].listView.firstClick == false) || 
            		(listViewArgs.listView && listViewArgs.listView.firstClick == false)) {
            } else {
            ...
            if (appendFields && !$.isFunction(appendFields) && !appendFields.noCompact && !uiCustom) {
            	$.each(appendFields,function(i,v){
            		if(!v.actionArray){ 
	            		var it = $('<td>');
	            		it.addClass("appendAction");
	               	 	it.append(
	               	 		$('<span style="cursor: pointer;">').addClass('last').html(_l(v.label))
	                          //  $('<span></span>').html("testtest")
	                        );
	               	 	it.appendTo($tr);
	               	 	//alert($('<span>').addClass('icon').html("test").end());
	               	 	//$('<span>').addClass('icon').html("test").end().appendTo(it);
	
	                    //    .appendTo($td)
            		}else{
            			var it = $('<td>');
	            		it.addClass("appendAction");
	            		var $tdspan = $('<span style="cursor: pointer;">').addClass('last');
	            		for(var i=0;i<v.actionArray.length;++i){
		               	 	$tdspan.html($tdspan.html()+"&nbsp;<a view='"+v.actionArray[i].view+"'>"+v.actionArray[i].label+"</a>");
	            		}
	            		it.append($tdspan);
	               	 	it.appendTo($tr);
            		}
                ...
                response: {
                    success: function(args) {
                        ...
                        addTableRows(preFilter, fields, args.data, $tbody, actions, {
                            ...
                            appendFields:options.appendFields/**add by Li Manxin */
                        });
    ...
    var makeListView = function($container, args, section) {
        ...
        var basicSearch = function() {
            ...
            loadBody(
                ...
                listViewData.actions, {
                    ...
                    detailView: listViewData.detailView,
                    setEndTable: setEndTable,
                    appendFields:listViewData.appendFields/**add by Li Manxin */
                }
            );
        };
        ...
        var advancedSearch = function(args) {
    		...
    		//同时选择开始时间，结束时间
            var paramData = args.data;
    		if(paramData.endDate!="" && paramData.startDate!=""){
	        		if(Date.parse(paramData.endDate)<Date.parse(paramData.startDate)){
	                    	cloudStack.dialog.notice({message:'结束时间必须大于等于开始时间'});
	                      	return;
	        			}
    		}
            page = 1;
    ...
    var prependItem = function(listView, data, actionFilter, options) {
        ...
        var $tr = addTableRows(
            ...
            targetArgs.actions, {
                ...
                appendFields:targetArgs.appendFields/**add by Li Manxin */
            }
        )[0];
    ...
    var replaceItem = function($row, data, actionFilter, after) {
        ...
        if(data!=undefined){//Modified by Li Manxin
        ...
        if (args == 'prependItem') {
            ...
            loadBody(
                ...
                listViewArgs.actions, {
                    ...
                    appendFields:listViewArgs.appendFields/**add by Li Manxin */
                }
            );
        } else {
            makeListView(
