...
(function($, cloudStack) {
    ...
    var makeNavigation = function(args) {
        ...
        /**
         * added by steven
         * 由于超级管理员权限菜单过多，如果是以超级管理员进入，默认将二级菜单隐藏以避免左侧拉得过长
         * update by huangxianjin
         */
        if(isAdmin()){
//        	$navList.find("li.network").hide();
//        	$navList.find("li.templates").hide();
//        	$navList.find("li.events").hide();
//        	$navList.find("li.projects").hide();
        	...
        	$navList.find("li.hpux").addClass(liClass).hide();
        	$navList.find("li.hphost").addClass(liClass).hide();
        	// "resourceMenu", "system", "storage", "network", "templates", "property", "equipmentEnroll"
        	$navList.find("li.system").addClass(liClass).hide();
//        	$navList.find("li.hpHostManagement").addClass(liClass).hide();
        	...
        	$navList.find("li.property").addClass(liClass).hide();
        	$navList.find("li.equipmentEnroll").addClass(liClass).hide();
        	
        	// "accounts", "domains", "projects", "affinityGroups", "global-settings", "resourcePoolPermission"
        	...
        	$navList.find("li.resourcePoolPermission").addClass(liClass).hide();
        	
        	// "events", "logs"
        	$navList.find("li.events").addClass(liClass).hide();
        	$navList.find("li.logs").addClass(liClass).hide();
        	
        	// "resourceStatistics", "workOrderReport", "topData"
        	$navList.find("li.resourceStatistics").addClass(liClass).hide();
        	$navList.find("li.workOrderReport").addClass(liClass).hide();
        	$navList.find("li.topData").addClass(liClass).hide();
        }
        return $navList;
    };
    ...
    var menuSlideToggle = function(sectionID) {
        ...
        if(sectionID=="instanceMenu"){
        	...
        	$("li.hpux").slideToggle("fast");
        	$("li.hphost").slideToggle("fast");
        	return true;
        }
...
//        	$("li.hpHostManagement").slideToggle("fast");
    ...
    var pageElems = {
        header: function(args) {
             ...
             * Date	2014/03/21
             */
            var $link=$('<div>').addClass("link").text('资源池页面');
    ...
    $.fn.cloudStack = function(args) {
        ...
        var $options = $('<div>').attr({
            ...
            .appendTo($('#header'));
        if(!isAdmin()){
        	$options.css("left","888px");
        }

        $(['label.logout']).each(function() {
            var $link = $('<a>')
                .attr({
                    href: '#'
                })
